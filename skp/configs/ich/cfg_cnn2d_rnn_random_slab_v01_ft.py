...
cfg.load_pretrained_model = os.path.join(
    cfg.save_dir,
    "ich.cfg_cnn2d_rnn_random_slab_v01",
    "9ca93faf/fold0/checkpoints/last.ckpt",
)
cfg.freeze_backbone = True
cfg.freeze_layers = ["linear_aux"]
...
cfg.loss_params = {"aux_weight": 0.0}

cfg.batch_size = 28
cfg.num_epochs = 10
cfg.optimizer = "AdamW"
cfg.optimizer_params = {"lr": 3e-5}
...
additional_targets = {f"image{idx}": "image" for idx in range(1, cfg.val_seq_len or cfg.seq_len)}
cfg.train_transforms = A.Compose(
    resize_transforms
    + [
        A.RandomCrop(
            height=int(cfg.image_height * 0.8125),
            width=int(cfg.image_width * 0.8125),
            p=1,
        )
    ]
    + [
