...
cfg.add_conv_head_pre_classifier = False
cfg.pretrained = True
...
cfg.fold = 0
cfg.dataset = "totalclassifier.seg_cls"
cfg.data_dir = (
...
cfg.loss_params = {
    ...
    "segmentation.FocalLossMemoryEfficient": {
        "params": {"gamma": 2.0, "ignore_background": True},
        "output_key": "seg",
        "weight": 100.0,
    },
}
...
augmentation_space = [
    # don't really need affines since using RandomResizedCrop
    # A.Affine(scale=(0.5, 1.5), border_mode=cv2.BORDER_CONSTANT, p=1),
    # A.Affine(
...
cfg.train_transforms = A.Compose(
    # since we are using masks and the classification label
    # is generated from the augmented mask, this should be OK
    [
        A.RandomResizedCrop(
            size=(cfg.image_height, cfg.image_width), scale=(0.4, 1.0), ratio=(1.0, 1.0)
        )
    ]
    + [
