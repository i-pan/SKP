...
cfg.batch_size = [128, 32, 14, 8]
cfg.num_epochs = [5, 5, 5, 5]
cfg.optimizer = "AdamW"
base_lr = 3e-4
cfg.optimizer_params = {"lr": [base_lr, base_lr, base_lr / 10, base_lr / 10]}
...
cfg.val_batch_size = cfg.batch_size[-1]
cfg.metrics = ["custom_cls.RANZCR_AUROC"]
cfg.val_metric = "auc_mean11"
cfg.val_track = "max"

cfg.image_height = [256, 512, 768, 1024]
cfg.image_width = [256, 512, 768, 1024]

cfg.list_of_resize_transforms = [
    A.Resize(h, w, p=1) for h, w in zip(cfg.image_height, cfg.image_width)
]
...
cfg.train_aug_transforms = [A.OneOf(augmentation_space, p=augment_p)]

cfg.train_transforms = A.Compose(
    [cfg.list_of_resize_transforms[0]] + cfg.train_aug_transforms, p=1
)

cfg.val_transforms = A.Compose([cfg.list_of_resize_transforms[0]], p=1)
