...
class Dataset(TorchDataset):
    def __init__(self, cfg: Config, mode: str):
        ...
        if "sampling_weight" in df.columns:
            self.sampling_weights = df.sampling_weight.tolist()
    ...
    def load_image(
        ...
        if not mask_present:
            return np.zeros(img_shape + (4,), dtype=np.uint8)
    ...
    def _get(self, i: int) -> Tuple[np.ndarray]:
        ...
        y = self.load_image(
            path=self.masks[i].replace("jpg", "png"),
            data_dir=self.cfg.seg_data_dir or self.cfg.data_dir,
            cv2_load_flag=cv2.IMREAD_UNCHANGED,
            mask_present=self.mask_present[i],
            img_shape=x.shape[:2],
        )
    ...
    def __getitem__(self, i: int) -> Dict[str, torch.Tensor]:
        ...
        input_dict = {
            ...
            "seg": {"x": x, "y": y, "mask_present": bool(self.mask_present[i])},
        }
        return input_dict
