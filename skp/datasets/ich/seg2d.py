"""
Loads 2D images and masks for intracranial hemorrhage segmentation.
"""
...
class Dataset(TorchDataset):
    ...
    def load_image(self, path: str, cv2_load_flag) -> np.ndarray:
        path = os.path.join(self.cfg.data_dir, path)
        img = cv2.imread(path, cv2_load_flag)
        return img

    def _get(self, i: int) -> Tuple[np.ndarray]:
        x = self.load_image(self.inputs[i], self.cfg.cv2_load_flag)
        if x.ndim == 2:
            x = rearrange(x, "h w -> h w 1")
        # assumes multiclass segmentation
        # if mask has multiple label channels, this won't work
        y = self.load_image(self.labels[i], cv2.IMREAD_GRAYSCALE)
        if self.cfg.combine_edh_and_sdh:
            # EDH- 1, IPH- 2, IVH- 3, SAH- 4, SDH- 5
            y[y == 5] = 1
        return x, y
