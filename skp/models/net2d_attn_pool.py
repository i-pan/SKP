...
class AttentionPooler(nn.Module):
    """
    Uses attention pooling as described in CoCa (https://arxiv.org/abs/2205.01917)
    to aggregate across a sequence of image features (tokens) and produce
    ...
    def __init__(
        ...
        # learnable query
        # num_query = 1 for global feature vector for classification tasks
        self.q_learn = nn.Parameter(torch.randn(1, embed_dim))
        self.norm_q = nn.LayerNorm(embed_dim) if norm_query else nn.Identity()
        self.norm_kv = nn.LayerNorm(embed_dim) if norm_context else nn.Identity()

    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor]:
        # x is shape (batch_size, num_features, embed_dim)
        q = self.q_learn.expand(x.shape[0], -1, -1)
        q = self.norm_q(q)
        x = self.norm_kv(x)
        out, a = self.attn(
...
class Net(nn.Module):
    def __init__(self, cfg: Config):
        ...
        self.attn = AttentionPooler(
            self.feature_dim,
            dropout=self.cfg.attn_dropout or 0.0,
            num_heads=self.cfg.attn_num_heads or 16,
        )
