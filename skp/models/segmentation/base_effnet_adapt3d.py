...
from skp.models.effnet_adapt3d import EffNetAdapt3d
...
class Net(nn.Module):
    def __init__(self, cfg: Config):
        super().__init__()
        self.cfg = cfg
        self.encoder = EffNetAdapt3d(cfg)
        ...
        output_size = (
            (self.cfg.dim0, self.cfg.dim1, self.cfg.dim2)
    ...
    def forward(
        ...
        x = self.normalize(x)
        feature_maps = self.encoder(x, return_feature_maps=True)
...
if __name__ == "__main__":
    ...
    model = Net(cfg)
    x = torch.randn((2, cfg.num_input_channels, cfg.dim0, cfg.dim1, cfg.dim2))
    out = model({"x": x})
