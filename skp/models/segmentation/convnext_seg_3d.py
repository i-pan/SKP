...
from skp.models.segmentation.decoders.convunext_3d import ConvUNeXtDecoder
...
class Net(nn.Module):
    def __init__(self, cfg: Config):
        ...
        encoder_args = {
            ...
            "temporal_strides": self.cfg.temporal_strides,
            "spatial_strides": self.cfg.spatial_strides,
            "block_kernel_size": self.cfg.block_kernel_size,
            "use_timm_weights": True if self.cfg.pretrained else False,
            "features_only": True,
            "in_chans": self.cfg.num_input_channels,
        }

        self.encoder = create_convnextv2_3d(**encoder_args)
        ...
        self.decoder = ConvUNeXtDecoder(self.cfg)
    ...
    def forward(
        ...
        # gradient checkpointing happens within decoder forward()
...
if __name__ == "__main__":
    ...
    cfg.temporal_strides = [4, 2, 2, 2]
    cfg.spatial_strides = [4, 2, 2, 2]
    cfg.block_kernel_size = [3, 7, 7]
    cfg.pretrained = True
    ...
    cfg.decoder_out_channels = [256, 64, 32, 16]
    cfg.seg_dropout = 0.1
    cfg.enable_gradient_checkpointing = True
    cfg.num_classes = 100

    cfg.output_size = cfg.num_slices, cfg.image_height, cfg.image_width
