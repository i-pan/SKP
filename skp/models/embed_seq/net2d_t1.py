...
class Net(nn.Module):
    def __init__(self, cfg: Config):
        ...
        self.cls_tokens = nn.Parameter(
            torch.randn((1, self.cfg.num_classes, self.feature_dim))
        )

        self.head = TransformerEncoderLayerWithAttnWeights(
...
if __name__ == "__main__":
    ...
    out = model({"x": x, "mask": torch.ones((2, 8)).bool()}, return_attn_weights=True)
    print(out["logits"].shape, out["attn_weights"].shape)
